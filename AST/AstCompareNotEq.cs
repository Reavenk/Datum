...
{
    public class AstCompareNotEq : AstCompare
    {
        public AstCompareNotEq(Val left, Val right)
        : base(left, right)
        { }
        ...
        public override Val Execute(Ctx ctx, out EvalTy evalTy)
        {
            ...
            if (lres.wrapType == Type.String || rres.wrapType == Type.String)
                return Val.Make(lres.GetString() != rres.GetString());

            if (lres.wrapType != rres.wrapType)
                return Val.Make(true);

            return Val.Make(!lres.Equivalent(rres));
        }
    }
}